...
async function synthesize(payload) {
  ...
  try {
    ...
    const selectedVoice = VOICE_LIST.find(
      (voice) =>
    ...
    if (!selectedVoice) {
      throw new Error(
        `Không tìm thấy voice phù hợp với gender: ${voice_styles.gender} và style: ${voice_styles.style}`
      );
    }
    ...
    for (const segment of segments) {
      ...
      console.log(
        `Segment ${segment.index} uploaded to Cloudinary successfully`
      );
      ...
      await newVoice.save();
      console.log(
        `Voice record for segment ${segment.index} created in database successfully`
      );
...
async function getPreview(gender, style, language) {
  ...
  try {
    if (!gender || !style || !language) {
      throw new Error(
        "Missing required parameters: gender, style, or language"
      );
    }

    // Find voice based on gender and style
    const selectedVoice = VOICE_LIST.find(
      (voice) => voice.gender === gender && voice.voice_style === style
    );

    if (!selectedVoice) {
      throw new Error(
        `Không tìm thấy voice phù hợp với gender: ${gender} và style: ${style}`
      );
    }
    ...
    if (language === "vi-VN") {
      previewText =
        "Đây là một đoạn nghe thử giọng nói của tôi. Thấy giọng tôi thế nào?";
    } else if (language === "en-US") {
      previewText = "This is a preview of my voice. How do I sound?";
    } else if (language === "fr-FR") {
      previewText = "C'est une présentation de ma voix. Comment ça sonne?";
    } else if (language === "es-ES") {
      previewText = "Este es un ejemplo de mi voz. ¿Cómo suena?";
    } else if (language === "de-DE") {
      previewText = "Dies ist ein Vorschau meiner Stimme. Wie klingt das?";
    }
